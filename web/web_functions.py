...
def bar_1D_web(dataset, fields_available):
    ...
    with st.expander("Legend Labels", expanded=False):
        ...
        l_source = c1.selectbox(
            "Source", options=["dataset", "custom"], key="1d_leg_src"
        )
        l_values = c2.selectbox(
            "Labels Values", options=params["fields"], key="1d_leg_val"
        )
        l_coloring_field = c3.selectbox(
            "Coloring Field", options=params["fields"], key="1d_leg_col"
        )
        l_casetype = c4.selectbox(
            "Case Type", options=["title", "upper", "original"], key="1d_leg_case"
        )

        c11, c12 = st.columns(2)
        l_title = c11.text_input("Legend Title", key="1d_leg_t")
        l_loc = c12.selectbox(
            "Legend Location",
        ...
        bbox_x = j1.number_input("X (Horizontal)", value=1.00, step=0.05, key="1d_bx")
        bbox_y = j2.number_input("Y (Vertical)", value=0.00, step=0.05, key="1d_by")
        bbox_w = j3.number_input("Width", value=0.30, step=0.05, key="1d_bw")
        bbox_h = j4.number_input("Height", value=1.00, step=0.05, key="1d_bh")
...
def bar_2D_web(dataset, fields_available):
    ...
    with st.expander("Legend Labels", expanded=False):
        c1, c2, c3, c4 = st.columns(4)
        l_source = c1.selectbox(
            "Source", options=["dataset", "custom"], key="2d_leg_src"
        )
        l_values = c2.selectbox(
            "Labels Values", options=params["fields"], key="2d_leg_val"
        )
        l_coloring_field = c3.selectbox(
            "Coloring Field", options=params["fields"], key="2d_leg_col"
        )
        l_casetype = c4.selectbox(
            "Case Type", options=["title", "upper", "original"], key="2d_leg_case"
        )

        c11, c12 = st.columns(2)
        l_title = c11.text_input("Legend Title", key="2d_leg_t")
        l_loc = c12.selectbox(
            "Legend Location",
            options=["upper left", "lower left", "upper right", "lower right"],
            key="2d_leg_l",
        ...
        st.caption("**BBox Anchor (Advanced Positioning)**: (x, y, width, height)")
        j1, j2, j3, j4 = st.columns(4)
        bbox_x = j1.number_input("X (Horizontal)", value=1.00, step=0.05, key="2d_bx")
        bbox_y = j2.number_input("Y (Vertical)", value=0.00, step=0.05, key="2d_by")
        bbox_w = j3.number_input("Width", value=0.30, step=0.05, key="2d_bw")
        bbox_h = j4.number_input("Height", value=1.00, step=0.05, key="2d_bh")
